...
action.run = async vars =>
  await new Promise((solve, reject) => {
    let [stdout, stderr] = ["", ""]
    const env = {...process.env, ...action.input(vars), GITHUB_REPOSITORY: "CryptoDragonLady/metrics"}
...
//Web instance
const web = {}
web.run = async vars => (await axios.get(`http://localhost:3000/CryptoDragonLady?${new url.URLSearchParams(Object.fromEntries(Object.entries(vars).map(([key, value]) => [key.replace(/^plugin_/, "").replace(/_/g, "."), value])))}`)).status === 200
web.start = async () =>
  new Promise(solve => {
    let stdout = ""
...
placeholder.run = async vars => {
  ...
  return typeof await placeholder({
    ...
    config,
    version: "TEST",
    user: "CryptoDragonLady",
    avatar: "https://github.com/CryptoDragonLady.png",
  }) === "string"
}
